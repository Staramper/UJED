<template>
    ...
    <!-- DIV DEL TITULO DEL NEGOCIO -->
        ...
        <div v-if="ModoB==1" class="text-center">
            <p class="display-4 text-primary shadow p-3">No hay ningun producto con esta categoria</p>
        </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                ...
                                <p class="card-text">{{ product.prd_desc }}</p>
                                <p class="card-text"><div class="fw-bold">Precio:</div>${{ product.prd_price }}</p>
                            </div>
                            <div class="row">
                                <div class="col text-center">
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#exampleModal" @click="vista(product)">
                                        Ver!
                                    </button>
                                </div>
                            </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                ...
                <div v-if="vendedor" class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">Vista previa del producto</h1>
                    ...
                    <div class="modal-body">
                        <div class="row text-center">
                            <div class="col">
                                <product-view
                                :pid="this.datosProducto.id"
                                :ptitle="this.datosProducto.title"
                                :pdesc="this.datosProducto.desc"
                                :pprice="this.datosProducto.price"
                                :pimg="this.datosProducto.img"
                                ></product-view>
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                actualBusiness: 0,
                socioBusiness: 0,
                datosProducto: {title:'',desc:'',price:'', img:''},
            }
        },
        methods: {
            // getBusiness() {
            ...
            // },
            vista(product){
                this.datosProducto = {id: product.id, title: product.prd_title, desc: product.prd_desc, price:product.prd_price, img: product.prd_picture};
            },
            mCarrito(){
                this.ModoB = 5;
            },
            ...
            // },
            getAllProduct() {
                var url = 'getAllProducto?categ=' + this.Categ;
                // console.log(url);
                axios.get(url).then(res => {
                    this.products = res.data;
                    this.products == '' ? this.ModoB = 1 : this.ModoB = 2
                    // console.table(this.products);
                    // console.table(this.ModoB);
                });
            },
            getQuejas() {
                this.showQ = true;
